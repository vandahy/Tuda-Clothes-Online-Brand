<template>
  <footer>
    <div class="logofooter-container">
      <img src="/images/logo.png" alt="Logo" />
    </div>
    <div class="footer-container">
      <div class="infomation">
        <h1 class="in4">Infomation</h1>
        <p>Founder: Văn Đahy</p>
        <p>27 Thien Ho Duong, Ward 1, Go Vap District, TP HCM</p>
        <p>Phone: 0903 123 456</p>
      </div>

      <div class="quick-link">
        <h1 class="quicklink">Quick Links</h1>
        <p>New Arrivals</p>
        <p>Shop</p>
        <p>Look-book</p>
        <p>News</p>
      </div>

      <div class="Follow-us">
        <h1 class="social">Follow Us On</h1>
        <a href="https://www.facebook.com/dihat77"
          ><i class="fa-brands fa-facebook"></i
        ></a>
        <a href="https://www.instagram.com/onlydihat07/"
          ><i class="fa-brands fa-square-instagram"></i
        ></a>
        <a
          href="https://www.tiktok.com/@07july.dihat?is_from_webapp=1&sender_device=pc"
          ><i class="fa-brands fa-tiktok"></i
        ></a>
      </div>
    </div>
    <div class="copyright">
      <span>Copyright © 2025 Dahy. All rights reserved.</span>
    </div>
  </footer>
</template>
